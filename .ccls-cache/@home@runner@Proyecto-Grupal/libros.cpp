...
bool guardarLibros(Tlibro libros[], int cantidadLibros,
  ...
  for (int i = 0; i < cantidadLibros; i++) {
    archivo << libros[i].id << ',' << libros[i].titulo << ',' << libros[i].autor
            << ',' << libros[i].anioPublicacion << ',' << libros[i].editorial
            << ',' << libros[i].alquilado
...
void agregarLibro() {
  ...
  setColor(LIGHTGREEN);
  leerTextoValido(librotemp.titulo, "Ingrese el título del libro: ",
                  "Título inválido. Ingrese nuevamente: ");
  leerTextoValido(librotemp.autor, "Ingrese el autor del libro: ",
                  "Autor inválido. Ingrese nuevamente: ");
  setColor(LIGHTGREEN);
  cout << "Ingrese el año de publicación: ";
  while (!(cin >> librotemp.anioPublicacion) ||
         !validarAnio(librotemp.anioPublicacion)) {
    setColor(RED);
  ...
  leerTextoValido(librotemp.editorial, "Ingrese la editorial del libro: ",
                  "Editorial inválida. Ingrese nuevamente: ");
  setColor(LIGHTGREEN);
  cout << "El libro está disponible para alquilar? (1 para sí, 0 para no): ";
  while (!(cin >> librotemp.alquilado) ||
         !validarAlquilado(librotemp.alquilado)) {
    setColor(RED);
...
void mostrarlibros() {
  ...
  while (true) {
    ...
    for (int i = inicio; i < fin; ++i) {
      ...
      cout << "Disponible para alquilar: "
           << (libros[i].alquilado ? "Si" : "No") << endl;
      cout << "---------------------------------\n";
    }
...
void leerTextoValidoOpcional(string &input, const string &mensaje,
                             const string &valorActual) {
...
void modificarLibro(int id) {
  ...
  cout << "Disponible para alquilar: "
       << (libros[posicion].alquilado ? "Sí" : "No") << endl;

  // Modificar los detalles del libro
  cout << "Ingrese los nuevos datos (presione enter para mantener el valor "
          "actual):"
       << endl;
  ...
  cin.ignore();
  leerTextoValidoOpcional(nuevoTitulo,
                          "Nuevo título: ", libros[posicion].titulo);
  ...
  if (!nuevoAnioStr.empty()) {
    ...
    if (!validarAnio(nuevoAnio)) {
      cout << "Año inválido. No se realizaron cambios en el año de publicación."
           << endl;
  ...
  leerTextoValidoOpcional(nuevaEditorial,
                          "Nueva editorial: ", libros[posicion].editorial);
  ...
  if (!nuevoEstadoStr.empty()) {
    ...
    if (!validarAlquilado(nuevoEstado)) {
      cout << "Entrada inválida. No se realizaron cambios en el estado de "
              "alquiler."
           << endl;
